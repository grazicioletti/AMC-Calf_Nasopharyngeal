...
# doing the linear mixed model first
library(lme4)
library(lmerTest)

alpha$Age_wk <- as.numeric(as.character(alpha$Age_wk))

lmm1 <- lmer(Shannon ~ Diet * Age_wk + (1 | Animal_ID), data = alpha)
...
kw_Shannon_Ab <- kruskal.test(Shannon ~ Age_wk, data = alpha_Ab) # Kruskal-Wallis chi-squared = 1.8624, df = 4, p-value = 0.7611
kw_Shannon_No <- kruskal.test(Shannon ~ Age_wk, data = alpha_No) # Kruskal-Wallis chi-squared = 1.3218, df = 4, p-value = 0.8577
...
lmm_obs <- lmer(Observed ~ Diet * Age_wk + (1 | Animal_ID), data = alpha)
...
kw_Observed_Ab <- kruskal.test(Observed ~ Age_wk, data = alpha_Ab) # Kruskal-Wallis chi-squared = 1.5372, df = 4, p-value = 0.82
kw_Observed_No <- kruskal.test(Observed ~ Age_wk, data = alpha_No) # Kruskal-Wallis chi-squared = 0.82064, df = 4, p-value = 0.9357
...
``` {r }
# ===========================================================
# SAVE REPRODUCIBILITY TABLES FOR GitHub — AMR PIPELINE
# ===========================================================

dir.create("amr", showWarnings = FALSE)

# Helper: Convert htest (Kruskal) to data frame
kw_to_df <- function(kw) {
  data.frame(
...
write.csv(contamdf.prev, "amr/decontam_results-amr.csv", row.names = TRUE)
writeLines(contams, "amr/list_of_contaminants-amr.txt")

# ===========================================================
# 2) Alpha diversity tables
# ===========================================================

write.csv(alpha, "amr/AMR-alpha.csv", row.names = FALSE)

# ---- Normality tests ----
sink("amr/AMR-shapiro_alpha.txt")
print(shapiro_shannon)
print(shapiro_observed)
sink()

# ---- Summary stats ----
write.csv(sum_Shannon, "amr/AMR-sum_Shannon.csv", row.names = FALSE)
write.csv(sum_Observed, "amr/AMR-sum_Observed.csv", row.names = FALSE)

# ===========================================================
# 3) Kruskal–Wallis (converted to data frames)
# ===========================================================

write.csv(kw_to_df(kw_Shannon_Ab), "amr/AMR-KW_Shannon_Ab.csv", row.names = FALSE)
write.csv(kw_to_df(kw_Shannon_No), "amr/AMR-KW_Shannon_No.csv", row.names = FALSE)
write.csv(kw_to_df(kw_Observed_Ab), "amr/AMR-KW_Observed_Ab.csv", row.names = FALSE)
write.csv(kw_to_df(kw_Observed_No), "amr/AMR-KW_Observed_No.csv", row.names = FALSE)

# ===========================================================
# 4) Linear Mixed Models
# ===========================================================

saveRDS(lmm1,     "amr/AMR-lmm_Shannon.rds")
saveRDS(lmm_obs,  "amr/AMR-lmm_Observed.rds")

# ===========================================================
# 5) Outliers
# ===========================================================

writeLines(as.character(shannon_upper_outliers),  "amr/AMR-outlier_Shannon_upper.txt")
writeLines(as.character(observed_upper_outliers), "amr/AMR-outlier_Observed_upper.txt")

# ===========================================================
# 6) Beta diversity / PERMANOVA
# ===========================================================

saveRDS(psdist_Ab, "amr/AMR-psdist_Ab.rds")
saveRDS(psdist_No, "amr/AMR-psdist_No.rds")

adonis_Ab <- adonis2(psdist_Ab ~ sample_data(beta1_Ab)$Age_wk, permutations = 10000)
adonis_No <- adonis2(psdist_No ~ sample_data(beta1_No)$Age_wk, permutations = 10000)

write.csv(as.data.frame(adonis_Ab), "amr/AMR-adonis_Ab.csv")
write.csv(as.data.frame(adonis_No), "amr/AMR-adonis_No.csv")
...
write.csv(as.data.frame(tax_table(noneg)), "amr/tax_table_cleaned.csv")
...
# ===========================================================
# 10) CLR abundance tables (ordination inputs)
# ===========================================================

clr_Ab <- abundances(beta1_Ab)
clr_No <- abundances(beta1_No)

write.csv(clr_Ab, "amr/AMR-clr_abundance_Ab.csv")
write.csv(clr_No, "amr/AMR-clr_abundance_No.csv")

# ===========================================================
# 11) Save final cleaned phyloseq object
# ===========================================================

saveRDS(noneg, "amr/noneg_amr_clean.rds")

cat("✓ All AMR output files successfully saved in /amr folder\n")
