...
shannon_lmm_df <- data.frame(
  Term   = rownames(lmm1_aov),
  Fvalue = round(lmm1_aov$`F value`, 4),
  df     = lmm1_aov$DenDF,
  Pvalue = signif(lmm1_aov$`Pr(>F)`, 3)
)

write.csv(shannon_lmm_df, "lmm_Shannon.csv", row.names = FALSE)

observed_lmm_df <- data.frame(
  Term   = rownames(lmm_obs_aov),
  Fvalue = round(lmm_obs_aov$`F value`, 4),
  df     = lmm_obs_aov$DenDF,
  Pvalue = signif(lmm_obs_aov$`Pr(>F)`, 3)
)

write.csv(observed_lmm_df,
          "lmm_Observed.csv",
          row.names = FALSE)
