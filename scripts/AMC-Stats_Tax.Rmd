...
kw_Shannon_Ab <- kruskal.test(Shannon ~ Age_wk, data = alpha_Ab)  # chi-squared = 0.66667, df = 4, p-value = 0.9554

kw_Shannon_No <- kruskal.test(Shannon ~ Age_wk, data = alpha_No) #  chi-squared = 3.497, df = 4, p-value = 0.4783
...
kw_Observed_Ab <- kruskal.test(Observed ~ Age_wk, data = alpha_Ab) # Kruskal-Wallis chi-squared = 2.1356, df = 4, p-value = 0.7108
kw_Observed_No <- kruskal.test(Observed ~ Age_wk, data = alpha_No) # KKruskal-Wallis chi-squared = 1.2327, df = 4, p-value = 0.8727
...
# Helper: Convert htest (from kruskal.test) into a data frame
...
write.csv(contamdf.prev, "taxonomic/decontam_results.csv", row.names = TRUE)
writeLines(contams_tax, "taxonomic/list_of_contaminants.txt")
...
# ---- Normality tests ----
sink("taxonomic/shapiro_alpha.txt")
print(shapiro_shannon)
print(shapiro_observed)
sink()

# ---- Kruskal–Wallis (converted to data frame) ----
write.csv(kw_to_df(kw_Shannon_Ab), "taxonomic/KW_Shannon_Ab.csv", row.names = FALSE)
write.csv(kw_to_df(kw_Shannon_No), "taxonomic/KW_Shannon_No.csv", row.names = FALSE)
write.csv(kw_to_df(kw_Observed_Ab), "taxonomic/KW_Observed_Ab.csv", row.names = FALSE)
write.csv(kw_to_df(kw_Observed_No), "taxonomic/KW_Observed_No.csv", row.names = FALSE)
...
# ---- Linear Mixed Models ----
saveRDS(lmm1,     "taxonomic/lmm_Shannon.rds")
saveRDS(lmm_obs,  "taxonomic/lmm_Observed.rds")

# ---- Outlier IDs ----
write.table(outlier_ids,
            "taxonomic/outlier_ids.txt",
            row.names = FALSE,
            col.names = FALSE,
            quote = FALSE)

# ===========================================================
# 3) Beta diversity inputs
# ===========================================================
...
# ===========================================================
# 6) CLR-transformed tables for ordination
# ===========================================================
...
# ===========================================================
# 7) Final cleaned phyloseq object
# ===========================================================

saveRDS(noneg_tax, "taxonomic/noneg_tax_clean.rds")

cat("✓ All TAXONOMY output files successfully saved in /taxonomic folder\n")
